...
create policy "owner_legal_document_emails_admin_read" on public.owner_legal_document_emails for select to authenticated using (public.has_role((select auth.uid()), 'admin'));
-- rpc_rate_limits deliberately has no client policy: only SECURITY DEFINER functions can access it.;
