...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (action === "start") {
      ...
      const { data: business } = await admin.from("businesses").select("id").eq("slug", shop).eq("is_active", true).maybeSingle();
    ...
    const [{ data: profile }, { data: memberships, error: membershipsError }] = await Promise.all([
      ...
      admin.from("memberships")
        .select("stamp_count,points_balance,visit_count,business:businesses(name,slug,loyalty_type)")
