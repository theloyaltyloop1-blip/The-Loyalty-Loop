...
Status: Fidel migrations are applied and reviewed, and `fidel-webhook` version 3 is approved. Genuine Test-mode auth and clearing deliveries are verified (own URL and key each, HTTP 200, clearing reuses the auth id, nothing awarded). Test mode sent no refund or negative-clearing webhook for a negative test transaction; this is most likely a Test-mode limitation. Refund delivery and correlation remain an open production risk, gated on one genuine live refund before real shoppers link cards. Next: the product owner sets up a dedicated test shop and account, then Codex runs the fixture stage (auth and clearing, no refund leg). **2026-09-25: first genuine end-to-end card payment credited automatically** (linked test card → Fidel Test-mode payment → £6.37 progress at the spend-based test shop Pure Elegant); manual £ entry is also live and device-tested.
...
- **Milestones 1 and 2 are complete.** The current product model is cumulative
  ...
  `originalTransactionId` when a refund matches a purchase. Signed sandbox
  deliveries still need to confirm the subscription-specific URL and secret mapping.
...
- Current stage: the role-detection correction is live and verified by Claude's
  rolled-back API-layer probes. The selector-routing fix passes local tests and remote
  probes. The owner created one transaction in Fidel Playground Test mode; the genuine
  auth signature passed, delivery returned HTTP 200, one auth event was ledgered and
  the handler reported `unknown_merchant`. Its redacted schema is in `CLAUDE_HANDOFF.md`.
  No fixtures or refunds. Clearing/refund signatures and correlation remain unverified.
  Full-history replay remains a pre-pilot gate requiring Docker or a preview branch.
...
files directly, so no uploads are needed. The current stage is milestone 5 review, so
