...
const KEY = 'loyalty_loop_biometric_lock'
...
export async function setBiometricLock(enabled: boolean) {
  ...
  const result = await LocalAuthentication.authenticateAsync({ promptMessage: 'Turn on app lock', promptDescription: 'Confirm to protect The Loyalty Loop.', biometricsSecurityLevel: 'strong' })
...
export async function unlockWithBiometrics() {
  const result = await LocalAuthentication.authenticateAsync({ promptMessage: 'Unlock The Loyalty Loop', promptDescription: 'Use Face ID, Touch ID or your fingerprint.', biometricsSecurityLevel: 'strong' })
