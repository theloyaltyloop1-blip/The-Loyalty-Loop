...
export function WhatsAppOnboarding() {
  ...
  if (error && !handoff) return <AuthLayout title="That link has expired"><p className="text-red-600 font-semibold">{error}</p><Link className="mt-5 inline-block font-bold text-primary" to="/">Back to The Loyalty Loop</Link></AuthLayout>
  if (!handoff) return null
  if (confirmationSent) return <AuthLayout title="Check your email"><p className="text-foreground font-semibold">We’ve sent a confirmation link to {handoff.email}. Open it to finish linking WhatsApp to your Loyalty Loop account.</p></AuthLayout>
