...
function StarRating({ rating }: { rating: number }) {
  return <div className="flex gap-0.5">{[1, 2, 3, 4, 5].map((item) => <Star key={item} className={'h-4 w-4 ' + (item <= rating ? 'fill-accent text-accent' : 'text-black/15')} />)}</div>
}
...
function ReviewCard({ review, ownerId, canRespond, onSaved }: { review: ShopReview; ownerId: string; canRespond: boolean; onSaved: () => void }) {
  ...
  return <article className="rounded-2xl bg-card p-6 shadow-[0_1px_3px_rgba(0,0,0,0.08)]"><div className="flex justify-between gap-4"><div><p className="font-bold text-foreground">Customer review</p><StarRating rating={review.rating} /></div><p className="text-xs text-foreground/40">{new Date(review.created_at).toLocaleDateString()}</p></div>{review.body && <p className="mt-3 text-foreground/70 whitespace-pre-wrap">{review.body}</p>}{review.reply && !editing ? <div className="mt-5 border-l-2 border-primary pl-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-bold">Reply from the shop</p>{canRespond && <button onClick={() => setEditing(true)} className="text-sm font-bold text-primary-hover">Edit</button>}</div><p className="mt-1 text-sm text-foreground/70 whitespace-pre-wrap">{review.reply.body}</p></div> : canRespond ? <form onSubmit={save} className="mt-5"><label className="block text-sm font-bold text-foreground mb-2">{review.reply ? 'Edit your reply' : 'Reply publicly'}</label><textarea value={reply} onChange={(e) => setReply(e.target.value)} maxLength={2000} placeholder="Thank them, answer their question, or make things right…" className="w-full min-h-24 rounded-xl border border-black/10 bg-white/70 px-3 py-2 outline-none focus:border-primary" />{error && <p className="mt-2 text-sm text-red-600">{error}</p>}<div className="flex gap-2 mt-3"><button disabled={saving || !reply.trim()} className="rounded-xl bg-primary px-4 py-2 text-sm font-bold text-white disabled:opacity-60">{saving ? 'Saving…' : review.reply ? 'Save reply' : 'Post reply'}</button>{review.reply && <button type="button" onClick={() => { setReply(review.reply?.body ?? ''); setEditing(false) }} className="rounded-xl border border-black/10 px-4 py-2 text-sm font-bold">Cancel</button>}</div></form> : null}</article>
}
