...
export async function acceptLegal(userId: string) { const rows = ['terms', 'privacy'].map((document_key) => ({ user_id: userId, document_key, document_version: '2026-09-23' })); const { error } = await supabase.from('legal_acceptances').upsert(rows, { onConflict: 'user_id,document_key,document_version' }); if (error) throw error }
