...
export function ReviewsSection({ businessId, userId, canReview }: { businessId: string; userId: string; canReview: boolean }) {
  ...
  return <section className="rounded-2xl bg-card shadow-[0_1px_3px_rgba(0,0,0,0.08)] p-6 mb-6">
    ...
    {canReview && <form onSubmit={save} className="rounded-xl bg-white/60 border border-black/7 p-4 mb-6"><p className="font-bold text-sm text-foreground mb-3">{mine ? 'Update your review' : 'Share your experience'}</p><Stars rating={rating} interactive onChange={setRating} /><label className="sr-only" htmlFor="review-body">Your review</label><textarea id="review-body" value={body} onChange={(e) => setBody(e.target.value)} maxLength={2000} placeholder="What did you enjoy?" className="mt-3 min-h-24 w-full rounded-xl border border-black/10 bg-white px-3 py-2 outline-none focus:border-primary" />{error && <p role="alert" className="mt-2 text-sm text-red-600">{error}</p>}<div className="flex gap-2 mt-3"><button data-press-feedback disabled={saving} className="rounded-xl bg-primary px-4 py-2 text-sm font-bold text-white disabled:opacity-60">{saving ? 'Saving…' : mine ? 'Update review' : 'Post review'}</button>{mine && <AlertDialog><Tooltip><TooltipTrigger asChild><AlertDialogTrigger asChild><button data-press-feedback type="button" aria-label="Delete review" disabled={saving} className="rounded-xl border border-red-200 px-3 py-2 text-red-600"><Trash2 aria-hidden="true" className="h-4 w-4" /></button></AlertDialogTrigger></TooltipTrigger><TooltipContent>Delete your review</TooltipContent></Tooltip><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete your review?</AlertDialogTitle><AlertDialogDescription>This can't be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => void remove()}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>}</div></form>}
